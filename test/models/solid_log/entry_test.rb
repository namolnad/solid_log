...
module SolidLog
  class EntryTest < ActiveSupport::TestCase
    ...
    test "by_time_range scope filters by time range" do
      old_entry = create_entry(created_at: 2.days.ago)
    ...
    test "recent scope orders by created_at desc" do
      first = create_entry(created_at: 3.hours.ago)
      second = create_entry(created_at: 2.hours.ago)
      third = create_entry(created_at: 1.hour.ago)

      results = Entry.recent.to_a

      assert_equal [third, second, first], results
    end
